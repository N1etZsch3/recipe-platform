...
import { 
  MessageSquare, Send, ArrowLeft, User, Search, AlertCircle,
  Mail, Heart, Bell, ChefHat, ChevronDown,
  CheckCircle, XCircle, Trash2, ArrowRight, MessageCircle, Edit3
} from 'lucide-vue-next'
import { 
  getConversations, getMessages, sendMessage as apiSendMessage, markRead,
  getMyComments, getRepliesForMe, getLikesForMe, getLikeDetail,
  checkOnlineStatus
} from '@/api/social'
...
// 筛选选项
const filterOptions = [
  { id: 'message', label: '私信', icon: Mail },
  { id: 'comment', label: '评论', icon: Edit3 },
  { id: 'reply', label: '回复', icon: MessageCircle },
...
const typeConfig = {
  ...
  comment: { label: '评论', color: 'bg-orange-500', textColor: 'text-orange-600', icon: Edit3 },
  ...
  myComments.value.forEach(c => {
    ...
    if (!commentsByRecipe[recipeId]) {
      commentsByRecipe[recipeId] = {
        ...
        recipeCoverImage: c.recipeCoverImage,
        comments: [],
        latestTime: null
      }
    }
    commentsByRecipe[recipeId].comments.push(c)
    const cTime = c.createTime ? new Date(c.createTime).getTime() : 0
    if (!commentsByRecipe[recipeId].latestTime || cTime > commentsByRecipe[recipeId].latestTime) {
      commentsByRecipe[recipeId].latestTime = cTime
    }
  })
  
  // 对每个菜谱的评论按时间降序排列
  Object.values(commentsByRecipe).forEach(group => {
    ...
    list.push({
      id: `comment-recipe-${group.recipeId}`,
      type: 'comment',
      recipeId: group.recipeId,
      avatar: group.recipeCoverImage,
      title: group.recipeTitle || '我的评论',
      subtitle: `${group.comments.length} 条评论`,
      time: formatTime(new Date(group.latestTime)),
      timestamp: group.latestTime || 0,
      unread: 0,
      raw: group
    })
  })
...
const currentFilter = computed(() => {
  return filterOptions.find(f => f.id === selectedType.value) || filterOptions[0]
})
...
const selectFilterType = (type) => {
  ...
  selectedItem.value = null  // 清空右侧内容
...
onMounted(async () => {
  ...
  if (urlType && ['message', 'comment', 'reply', 'like', 'system'].includes(urlType)) {
  ...
  if (chatWith) {
    selectedType.value = 'message' // 确保切换到私信分类
    ...
    <div class="max-w-5xl mx-auto px-4">
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex h-[calc(100vh-64px-32px)]">
          ...
          <div class="p-3 bg-white border-b border-gray-100">
            <div class="flex items-center justify-between mb-3">
              ...
              <div class="relative">
                ...
                >
                  <component :is="currentFilter.icon" class="w-4 h-4 text-gray-600" />
                  <span class="font-bold text-gray-800">{{ currentFilter.label }}</span>
                  <ChevronDown :class="['w-4 h-4 text-gray-400 transition-transform', showTypeDropdown ? 'rotate-180' : '']"/>
                </button>
                <!-- 下拉菜单 -->
                <div 
                  v-if="showTypeDropdown"
                  class="absolute top-full left-0 mt-1 w-40 bg-white rounded-lg shadow-lg border border-gray-100 py-1 z-20"
                >
                  <button
                    v-for="option in filterOptions"
                    :key="option.id"
                    @click="selectFilterType(option.id)"
                    :class="[
                      'w-full flex items-center gap-2 px-3 py-2 text-sm transition',
                      selectedType === option.id ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-50'
                    ]"
                  >
              ...
              <span class="text-xs text-gray-400">{{ filteredList.length }} 条</span>
            </div>
            <div class="relative">
        ...
        <div :class="[
          ...
          <template v-else-if="selectedItem.type === 'message'">
            ...
            <div class="flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-100">
              ...
              <img 
                ...
                class="w-10 h-10 rounded-full object-cover bg-gray-200 cursor-pointer hover:ring-2 hover:ring-blue-300 transition"
                @click="router.push(`/user/${selectedItem.userId}`)"
              >
              <div class="flex-1">
                <div 
                  class="font-medium text-gray-800 cursor-pointer hover:text-blue-500 transition"
                  @click="router.push(`/user/${selectedItem.userId}`)"
          ...
          <!-- ===== 评论详情 ===== -->
          <template v-else-if="selectedItem.type === 'comment'">
            <div class="flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-100">
              <button @click="backToList" class="md:hidden p-1.5 hover:bg-gray-100 rounded-lg transition">
                <ArrowLeft class="w-5 h-5 text-gray-600" />
              </button>
              <span class="font-medium text-gray-800">我在该菜谱的评论</span>
            </div>
            <div class="flex-1 overflow-y-auto p-4">
              ...
              <div class="bg-white rounded-xl p-4 shadow-sm mb-4">
                <div class="flex items-center gap-3">
                  <img 
                    v-if="selectedItem.raw.recipeCoverImage"
                    :src="selectedItem.raw.recipeCoverImage" 
                  ...
                  <div class="flex-1">
                    <div class="font-medium text-gray-800">{{ selectedItem.raw.recipeTitle }}</div>
                    <div class="text-xs text-gray-400">共 {{ selectedItem.raw.comments?.length || 0 }} 条评论</div>
                  </div>
                  <button 
              ...
              <div class="space-y-3">
                <div 
                  v-for="comment in selectedItem.raw.comments" 
                  :key="comment.id"
                ...
                >
                  <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="flex items-center gap-3 text-xs text-gray-400">
                      <span class="flex items-center gap-1"><Heart class="w-3 h-3" /> {{ comment.likeCount || 0 }}</span>
                      <span v-if="!comment.parentId" class="flex items-center gap-1"><MessageCircle class="w-3 h-3" /> {{ comment.replyCount || 0 }}</span>
                    </div>
                  </div>
                  <p class="text-gray-700 text-sm">{{ comment.content }}</p>
                  <div v-if="comment.parentId && comment.replyToUserName" class="mt-2 text-xs text-gray-400 bg-gray-50 px-2 py-1 rounded">
                    回复 <span class="text-orange-500">@{{ comment.replyToUserName }}</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="bg-white rounded-xl p-4 shadow-sm">
                <div class="flex items-start gap-3 mb-4">
                  <img 
                    ...
                    class="w-12 h-12 rounded-full object-cover cursor-pointer hover:ring-2 hover:ring-green-300 transition"
                    @click="router.push(`/user/${selectedItem.raw.replyUserId}`)"
                  >
                  <div class="flex-1">
                    <div 
                      class="font-medium text-gray-800 mb-1 cursor-pointer hover:text-green-500 transition"
                      @click="router.push(`/user/${selectedItem.raw.replyUserId}`)"
                    >{{ selectedItem.raw.replyUserName }}</div>
